...
<h3>부서번호에 따라 전체 사원 조회 또는 부서별 사원 조회</h3>
	<form action="index.jsp" id="frm">
		...
		<select name="deptno">
			...
			<option value="1"${ param.deptno eq "1" ? " selected='selected'":"" }>10번 부서</option>
			<option value="2"${ param.deptno eq "2" ? " selected='selected'":"" }>20번 부서</option>
			<option value="3"${ param.deptno eq "3" ? " selected='selected'":"" }>30번 부서</option>
		</select>
	...
	<c:if test="${ not empty param.deptno }">
		...
		pageContext.setAttribute("list",ss.dynamicChoose(deptno));
		%>
